#!/usr/bin/env bash
# web-client/update.sh
# Converted from update.ps1 to a Debian-compatible Bash script
# Usage: ./update.sh
...
info "Updating Web Client..."
info "Building Docker image..."

# Configuration
PROJECT_ID="carelink-web"
IMAGE_NAME="carelink-client"
...
# Determine the directory of this script (the web-client folder)
...
if [ ! -f "${DOCKERFILE_PATH}" ]; then
  fail "Dockerfile not found at ${DOCKERFILE_PATH}. Please run the script from the repo root, or place this script in the web-client folder."
fi
...
GCLOUD_ARGS=(run deploy "${IMAGE_NAME}" --image "gcr.io/${PROJECT_ID}/${IMAGE_NAME}:${TAG}" --region "${REGION}" --allow-unauthenticated --platform managed --memory 256Mi --cpu 1)
